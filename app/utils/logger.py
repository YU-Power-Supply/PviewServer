...
async def api_logger(request: Request, response=None, error=None):
    ...
    # body = await request.json() <- 바디 사용하면 에러뜨는데 원인을 모르겠음 ㅠㅠ
    if error:
        if request.state.inspect:  # inspect는 파이썬 코드 상에서 몇번째 줄에서 에러가 났는지 기록해줌
    ...
    # 로그가 많이 발생하는 서비스의 경우 json으로 저장하면 비용이 상당함. 그럴 땐 텍스트로 저장
    log_dict = dict(
    ...
    else:  # 400 이하는 서버에서 핸들링 된 에러니 info로 찍음
